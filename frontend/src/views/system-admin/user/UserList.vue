<template>
  <div class="page-container">
    ...
    <div class="content-card">
      ...
      <DataTable
        ...
        @update:selectedRows="handleSelectedRowsChange"
        v-model="internalRowSelectionState"
        :sticky-header="true"
        :sticky-columns="['status', 'first_name']"
        :loading="isLoading"
        :toggable-column-visibility="true"
        @update:visibleColumns="handleVisibleColumns"
      >
    ...
    <div
      ...
      class="mt-4 p-4 border rounded-md bg-blue-50"
    >
      ...
      <button @click="clearSelection" class="btn-primary mt-2">
        Clear Selection
      </button>
    </div>
  </div>
  ...
  <UserForm
    ...
    :initial-user="userDetails"
  />
</template>

<script setup>
import { ref, computed, watch, h, onMounted } from "vue";
import DataTable from "@/components/Table/VueTanstackTable/DataTable.vue";
...
const data = ref([]); // Reactive data array for the table
const loadDataSimulated = () => {
  isLoading.value = true; // Set loading to true
  data.value = []; // Clear current data if any, for better visual effect
...
const columns = ref([
  ...
  { label: "Emergency Contact #", field: "contact" },
  { label: "Created Date", field: "created_at" },
  { label: "Last Logged In", field: "last_login_at" },
...
const formattedData = computed(() =>
  data.value.map((data) => formatData(data))
);
...
function handleVisibleColumns(columns) {
  visibleColumns.value = columns;
  // You can now use visibleColumns.value in your parent component
  // for any logic that depends on which columns are currently displayed.
}
